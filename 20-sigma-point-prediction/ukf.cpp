 ...
 ******************************************************************************/
 ...
 Xsig_pred.setZero(n_x, 2 * n_aug + 1);
  Xsig_pred.col(0) = Xsig_aug.col(0).head(5);
  for (int col = 0; col <= 2 * n_aug; col++) {
    ...
    if (yaw_dot != 0) {
      x_pred << v / yaw_dot * (sin(yaw + yaw_dot * delta_t) - sin(yaw)),
          v / yaw_dot * (-cos(yaw + yaw_dot * delta_t) + cos(yaw)),
          0, yaw_dot * delta_t, 0;
    } else {
      x_pred << v * cos(yaw) * delta_t, v * sin(yaw) * delta_t, 0, 0, 0;
    }
    VectorXd x_noise = VectorXd(5);
    x_noise << 0.5 * delta_t * delta_t * cos(yaw) * nu_a,
        0.5 * delta_t * delta_t * sin(yaw) * nu_a,
        delta_t * nu_a,
        0.5 * delta_t * delta_t * nu_yaw_dotdot,
